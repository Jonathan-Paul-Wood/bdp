...
This is a small Firefox WebExtension template with a native sidebar UI. Clicking the toolbar button opens the Hello World sidebar. The panel starts with:

```html
<h1>hello world</h1>
```

Submitting the form saves the submitted value with `browser.storage.local` and updates the heading with `textContent`.
